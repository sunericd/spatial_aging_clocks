...
def get_paired_proximity_labels (adata, cutoff, celltype, cutoff_multiplier=1, region_obs="region", celltype_obs="celltype", animal_obs="mouse_id"):
    '''
...
def get_mechanism_stats_df (adata, near_ages, cutoff_multiplier, celltype, ct, full_adata, label=None,
                  min_pairs=50, obs_cols=[], genes=False, method="wilcoxon"):
    '''
    PRELIMINARY FUNCTION [will be removed in future iterations]
    '''
    if method == "wilcoxon":
        stat_label = "u"
    elif method == "ttest":
    ...
    if genes is True:
        # run differential gene expression analysis between Near and Far
        for gene in adata.var_names:
            
            near_exp = np.array(adata[:,gene].X).flatten()[near_ages=="Near"]
            far_exp = np.array(adata[:,gene].X).flatten()[near_ages=="Far"]
            n1 = np.sum(~np.isnan(near_exp))
            n2 = np.sum(~np.isnan(far_exp))
            
            # statistical test
            if method == "wilcoxon":
                t,p = mannwhitneyu(near_exp, far_exp, nan_policy='omit')
            elif method == "ttest":
                t,p = ttest_ind(near_exp, far_exp, nan_policy='omit')
                
            # effect size
            if (n1 > min_pairs) and (n2 > min_pairs):
                mean_diff = np.nanmean(near_exp)-np.nanmean(far_exp)
                pooled_sd = np.sqrt((n1*np.nanstd(near_exp)**2 + n2*np.nanstd(far_exp)**2) / (n1+n2)) 
                effect_size = mean_diff/pooled_sd # cohen's d = mean_diff/pooled_sd
            else:
                effect_size = np.nan
            ...
            # compile stats
            if label is None:
                row_df = pd.DataFrame([[celltype, ct, cutoff_multiplier, t, p, effect_size, near_freq, gene]],
                                  columns=["Near Cell", "AgeAccel Cell", "n", stat_label, "p", "Near Effect", "Near Freq", "Feature"])
            else:
                row_df = pd.DataFrame([[label, celltype, ct, cutoff_multiplier, t, p, effect_size, near_freq, gene]],
                                  columns=["label", "Near Cell", "AgeAccel Cell", "n", stat_label, "p", "Near Effect", "Near Freq", "Feature"])
            df = pd.concat((df,row_df))
    
    else: # cycle through features in adata.obs
        for obs_col in obs_cols:
                    
            near_exp = np.array(adata.obs[obs_col]).flatten()[near_ages=="Near"]
            far_exp = np.array(adata.obs[obs_col]).flatten()[near_ages=="Far"]
            n1 = np.sum(~np.isnan(near_exp))
            n2 = np.sum(~np.isnan(far_exp))
            ...
            # compile stats
            if label is None:
                row_df = pd.DataFrame([[celltype, ct, cutoff_multiplier, t, p, effect_size, near_freq, obs_col]],
                                  columns=["Near Cell", "AgeAccel Cell", "n", stat_label, "p", "Near Effect", "Near Freq", "Feature"])
            else:
                row_df = pd.DataFrame([[label, celltype, ct, cutoff_multiplier, t, p, effect_size, near_freq, obs_col]],
                                  columns=["label", "Near Cell", "AgeAccel Cell", "n", stat_label, "p", "Near Effect", "Near Freq", "Feature"])
            df = pd.concat((df,row_df))
