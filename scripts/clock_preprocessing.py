...
def normalize_adata (adata, zscore=True):
    ...
    # Normalization of data by volume -- actually this is already done in the raw data
...
def spatial_smoothing(adata, graph_method="fixed", adjacency_method="binary", label="predicted_age",
                      n_neighbors=15, alpha=0.1, max_iter=100, tol=1e-3):
    '''
    Spatial smoothing for labels (i.e. predicted age)
    
    Refer to spatial_propagation.py for argument descriptions
    '''
    # build graph and adjacency matrix
    build_spatial_graph(adata, method=graph_method, n_neighbors=n_neighbors)
    calc_adjacency_weights(adata, method=adjacency_method)
    
    X = np.array(adata.obs[label])
    S = adata.obsp["S"]
    
    # Propagate predictions
    Xp = propagate (X, S, alpha, max_iter=max_iter, tol=tol, verbose=True)
