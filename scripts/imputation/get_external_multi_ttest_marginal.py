...
parser.add_argument("margin_freq_cutoff", help="margin_freq_cutoff", type=int)
...
parser.add_argument("sig_dirpath", help="sig_dirpath to save")
...
group2 = args.group2
margin = args.margin
margin_freq_cutoff = args.margin_freq_cutoff
k_gene = args.k_gene
k_cell = args.k_cell
if k_gene != 'auto':
    k_gene = int(k_gene)
...
for method in methods:
    ...
    # (3) run DGEA with MI -- marginalized format
    adata.obs["marginal_condition"] = adata.obs[condition]+adata.obs[margin] # new condition metadata
    
    unique_margins, margin_freq = np.unique(adata.obs[margin], return_counts=True)
    unique_margins = unique_margins[margin_freq > margin_freq_cutoff] # get unique margin appendices
    
    for mgn in unique_margins: # run for each unique margin
        keys_list = multiple_imputation_testing (adata, predicted, calib_genes, "marginal_condition", n_imputations=n_imputations,
                                                 group1=group1+mgn, group2=group2+mgn, symmetric=symmetric, return_keys=True,
                                                 save_mi=False)

# Save results
try:
    adata.write(savedir+"/"+dataset_name+"_"+"_".join(methods)+f"_MI_EXTERNAL_TTEST_{condition}_{group1}_marginalizedOn{margin}.h5ad")
    # if error loading (i.e. metadata too large), then large_save instead
    try:
        adata2 = sc.read_h5ad(savedir+"/"+dataset_name+"_"+"_".join(methods)+f"_MI_EXTERNAL_TTEST_{condition}_{group1}_marginalizedOn{margin}.h5ad")
    except:
        large_save(adata, savedir+"/"+dataset_name+"_"+"_".join(methods)+f"_MI_EXTERNAL_TTEST_{condition}_{group1}_marginalizedOn{margin}")
        os.remove(savedir+"/"+dataset_name+"_"+"_".join(methods)+f"_MI_EXTERNAL_TTEST_{condition}_{group1}_marginalizedOn{margin}.h5ad")
# if error loading (i.e. metadata too large), then large_save instead
except:
    large_save(adata, savedir+"/"+dataset_name+"_"+"_".join(methods)+f"_MI_EXTERNAL_TTEST_{condition}_{group1}_marginalizedOn{margin}")
    os.remove(savedir+"/"+dataset_name+"_"+"_".join(methods)+f"_MI_EXTERNAL_TTEST_{condition}_{group1}_marginalizedOn{margin}.h5ad")